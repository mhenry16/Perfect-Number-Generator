...
;For the example the prompt gives the command line should be (perfectNumbers 0 0 3)
(define (perfectNumbers x start stop)
  (if (not(= start stop))
    (if (isPerfect x)
        (perNum x start stop)
        (perfectNumbers (+ x 2) start stop)))
  (void))
...
(define (perNum x start stop)
  ...
  (perfectNumbers (+ x 2)(+ start 1)stop))
...
;Main function which is ran to evaluate whether numbers are perfect, abundant, or deeficient (part 2)
...
(define (project3 x start stop)
  (if (not (= start stop))
      (cond [(isDeficient x) (defNum x start stop)]        
        [(isAbundent x) (abunNum x start stop)]
        [(isPerfect x) (perNum1 x start stop)]
        ))
      (void))
